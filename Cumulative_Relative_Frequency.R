...
#tuong doi tich luy cua tung ngày
#day5
countd5 = nrow(subset(day5,Temp>0 & Humi>0))
#day7
countd7 = countd5 + nrow(subset(day7,Temp>0 & Humi>0))
#day9
countd9 = countd7 + nrow(subset(day9,Temp>0 & Humi>0))
#day10
countd10 = countd9 + nrow(subset(day10,Temp>0 & Humi>0))

count = c(100, countd9/countd10*100, countd7/countd10*100, countd5/countd10*100)
#dem sô c???t tích luy m???i ngày
...
dayofmonth = as.factor(as.character(dayofmonth))#chuy???n v??? factor d??? d??? s???p x???p

caculate= data.frame(count, dayofmonth)
#d???o thành dataframe mói

ggplot(data=caculate,aes(x= dayofmonth, y= count)) +#dinh dang x và y
  geom_histogram(stat='identity',fill="blue", color = 'green', binwidth = 0.75)+
  geom_text(aes(label = count), vjust = -0.25)+#hi???n s??? trên d???u m???i c???t
  labs(x='Time',y='Relative Frequency (%)',title= 'Cumulative Relative Frequency Histogram' )+
  theme_bw()